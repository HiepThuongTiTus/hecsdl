...
UPDATE V_KHACHHANG
SET NgayDat='2015/06/15' WHERE NgayDat='2014/06/15'
...
--hàng của tất cả các lần đặt hàng trong năm 2014, mã khách hàng sẽ được truyền vào thông qua tham số đầu vào của hàm.
--Cụ thể như sau:
...
begin
	...
	if exists (select KHACHHANG.MaKH from KHACHHANG 
	join DONHANG on KHACHHANG.MaKH=DONHANG.MaKH 
	join CHITIETDONHANG on DONHANG.MaDH=CHITIETDONHANG.MaDH 
	where KHACHHANG.MaKH=@MaKH and YEAR(DONHANG.NgayDat)='2014'
...
begin
	begin tran deletepro;
		...
		select MaSP, MaDH from CHITIETDONHANG
		open contro
