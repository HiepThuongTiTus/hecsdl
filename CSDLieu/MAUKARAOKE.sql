...
--c2
SELECT a.SokhachToiDa FROM PHONG a
...
SELECT *FROM DICHVU
WHERE (DonViTinh='Chai' AND DonGia<20.000) AND (DonViTinh='Lon' AND DonGia>30.000)
...
SELECT COUNT(SoLuong) AS CHITIET_SUDUNGDV FROM HOADON
WHERE (TrangThai='Chua thanh toan' AND ThoiGianBatDauSD='DATE()')
...
WHERE EXISTS (
SELECT *FROM HOADON HD
WHERE (HD.MaKH = KH.MaKH AND ThoiGianBatDauSD='2014')
) AND NOT EXISTS (
SELECT *FROM HOADON HD
WHERE (HD.MaKH = KH.MaKH AND ThoiGianBatDauSD='2015')
)
...
WHERE EXISTS (
	SELECT *FROM HOADON hd
	WHERE(hd.ThoiGianBatDauSD='2014' AND hd.DonGia>50.000)
)
...
AND(
SELECT DISTINCT SD.SoLuong FROM CHITIET_SUDUNGDV SD
)
...
UPDATE HOADON HD SET HD.TrangThai='Da het han'
INNER JOIN KHACHHANG KH  ON KH.MaKH=HD.MaKH
WHERE KH.DiaChi='Hai Chau' AND HD.ThoiGianKetThucSD<'31/12/2015'
...
UPDATE PHONG p SET p.MoTa='Duoc su dung nhieu'
AND(
SELECT *FROM CHITIET_SUDUNGDV SD, HOADON HD
WHERE (SD.SoLuong>5 AND HD.ThoiGianBatDauSD='5/2015')
)
